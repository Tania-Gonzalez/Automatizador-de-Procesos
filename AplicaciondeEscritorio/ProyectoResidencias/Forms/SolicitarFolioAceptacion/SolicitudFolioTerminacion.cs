...
{
    public partial class SolicitudFolioTerminacion : Form
    {
        #region Variables
        ...
        #endregion
        public SolicitudFolioTerminacion(string a, List<int> b)
        {
            InitializeComponent();
            ...
            using (var context = new Base_Servicio_PracticasEntities(cadenaconexion))
            {
                var listabusqueda = (from e in context.Estudiante
                                     ...
                                     select new SolicitudFolioAceptacionList
                                     {
                                         ID = e.ID_Estudiante,
                                         Numero = "",
                                         Asunto = "Terminación de " + pg.Nombre_Programa + " para " + dp.Nombre1 + " " + dp.Nombre2 + " " + dp.Apellido_Paterno + " " + dp.Apellido_Materno,
        ...
        private void btnRegistrar_Click(object sender, EventArgs e)
        {
            using (var context = new Base_Servicio_PracticasEntities(cadenaconexion))
            {
                foreach (var variable in lista)
                {
                    ...
                    if (registro != null)
                    {
                        registro.Peticion_Folio_Carta_Terminacion = true;
                        registro.Fecha_Peticion_Folio_Carta_Terminacion = DateTime.Today;
                        context.SaveChanges();
                    }
                }
