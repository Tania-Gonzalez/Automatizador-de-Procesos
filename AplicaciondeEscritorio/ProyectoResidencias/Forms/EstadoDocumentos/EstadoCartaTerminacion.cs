...
{
    public partial class EstadoCartaTerminacion : Form
    {
        #region Variables
        ...
        private void BtnDatosPersonales_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedIndex == 0)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Carta_Terminacion = 0;
                        a.Fecha_Peticion_Carta_Terminacion = null;
                        a.Fecha_Llegada_Carta_Terminacion = null;
                        context.SaveChanges();
                    }
            ...
            if (comboBox1.SelectedIndex == 1)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Carta_Terminacion = 1;
                        a.Fecha_Peticion_Carta_Terminacion = DateTime.Today;
                        context.SaveChanges();
                    }
                    MessageBox.Show("Modificacion Correcta");
                    Cargar();
            ...
            if (comboBox1.SelectedIndex == 2)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Carta_Terminacion = 2;
                        a.Fecha_Llegada_Carta_Terminacion = DateTime.Today;
                        context.SaveChanges();
                    }
                    MessageBox.Show("Modificacion Correcta");
                    Cargar();
        ...
        public EstadoCartaTerminacion(string a, int b)
        {
            InitializeComponent();
            this.ID_Est = b;
        ...
        private void Cargar()
        {
            ...
            using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
            {
                ...
                {
                    edo = valores.Peticion_Carta_Terminacion;
                }
            }
            if (edo == 0)
            {
