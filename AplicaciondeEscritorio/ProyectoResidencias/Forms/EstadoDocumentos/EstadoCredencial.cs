...
{
    public partial class EstadoCredencial : Form
    {
        #region Variables
        ...
        private void BtnDatosPersonales_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedIndex == 0)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Credencial = 0;
                        a.Fecha_Peticion_Credencial = null;
                        a.Fecha_Llegada_Credencial = null;
                        context.SaveChanges();
                    }
            ...
            if (comboBox1.SelectedIndex == 1)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Credencial = 1;
                        a.Fecha_Peticion_Credencial = DateTime.Today;
                        context.SaveChanges();
                    }
                    MessageBox.Show("Modificacion Correcta");
                    Cargar();
            ...
            if (comboBox1.SelectedIndex == 2)
            {
                using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
                {
                    ...
                    if (a != null)
                    {
                        a.Peticion_Credencial = 2;
                        a.Fecha_Llegada_Credencial= DateTime.Today;
                        context.SaveChanges();
                    }
                    MessageBox.Show("Modificacion Correcta");
                    Cargar();
        ...
        public EstadoCredencial(string a, int b)
        {
            InitializeComponent();
            this.ID_Est = b;
        ...
        private void Cargar()
        {
            ...
            using (var context = new Base_Servicio_PracticasEntities(CadenaConexion))
            {
                ...
                {
                    edo = valores.Peticion_Credencial;
                }
            }
            if (edo == 0)
            {
